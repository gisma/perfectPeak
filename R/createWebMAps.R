...
createWebMaps<-function(sp,title='Independence Measures',base=c("osm","mqosm","mqsat","water","toner","tls"),sty=sty,ctl=NA){
  sp.df<-data.frame(sp)
  sp.df$color <- heat.colors(5)
  data.frame(sp.df)<- ~xcoord+ycoord
  map<-webmap(layer(layerData = sp.df,
               name = "prominence",
               lstyle(
                 pointRadius = '${prominence}',
                 fillColor = "${color}",
                 strokeColor = "black",
                 fillOpacity = 0.4)),
         title="Prominence",
         htmlFile="prominence.html",
         browse=TRUE,
         toShiny=FALSE)
